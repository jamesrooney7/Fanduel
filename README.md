...
| **A "press & hold" / "verify you're human" challenge appears** | Solve it by hand in the browser window — the tool waits (up to ~3 min) and continues automatically once the page loads. It uses a saved browser profile (`~/.fanduel_scraper/chrome-profile`), so you usually won't be challenged again for a while. Tip: run with `--headed` so the window is visible from the start. |
| **"no market data ... was captured"** | Either the game isn't live/open for betting, or a headless run got challenged. Re-run with `--headed`; if a challenge appears, solve it. Add `--dump-raw dumps/` and check `dumps/page.png` to see what the browser showed. |
